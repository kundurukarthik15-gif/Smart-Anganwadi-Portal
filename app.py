...
def register():
    """Public registration — requires center details (name, district, mandal, village)."""
    data = request.get_json() or {}
    ...
    token = make_token(u["id"], u["center_id"], u["email"])
    return ok({"token": token, "user": u}, "Account created", 201)
