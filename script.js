...
async function doLogin() {
  ...
  const btn = document.querySelector('.btn-login-submit') || document.querySelector('.btn-login');
...
async function doRegister() {
  ...
  if (res.success) {
    toast(`✅ Account created for ${name}! Signing you in…`, 'success');
    localStorage.setItem('token', res.data.token);
    ...
    if (profile.success) {
      centerInfo = {
        center_name: profile.data.center?.center_name || centerName,
        village: profile.data.center?.village || village,
        mandal: profile.data.center?.mandal || mandal,
        district: profile.data.center?.district || district,
        address: profile.data.center?.address || address
      };
    }
    ...
    DB.user = {
        ...
        email: res.data.user.email,
        name: res.data.user.full_name,
        center: centerInfo.center_name,
        center_id: res.data.user.center_id,
        village: centerInfo.village,
        mandal: centerInfo.mandal,
...
async function openPortal() {
  ...
  document.getElementById('sbu-name').textContent    = u.name;
  document.getElementById('sbu-center').textContent  = u.center;
  if (document.getElementById('sbu-village')) document.getElementById('sbu-village').textContent = u.village || '';
  if (document.getElementById('sbu-role'))    document.getElementById('sbu-role').textContent    = u.role    || '';
  document.getElementById('tb-avatar').textContent   = av;
  document.getElementById('tb-uname').textContent    = u.name.split(' ')[0];
  document.getElementById('tb-ucenter').textContent  = u.center;
...
function renderDashboard() {
  ...
  setEl('today-date2', new Date().toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' }));

  // ── 6 Stat cards
  const statsData = [
    { icon: 'bi-people-fill',             bg: 'bg-p',  col: 'col-p',  val: totalKids,      lbl: 'Total Children',        link: 'children' },
    { icon: 'bi-person-heart',            bg: 'bg-pk', col: 'col-pk', val: totalBenef,     lbl: 'Total Beneficiaries',   link: 'beneficiary' },
    { icon: 'bi-file-earmark-bar-graph-fill', bg: 'bg-t', col: 'col-t', val: totalVillagers,lbl: 'Total Villagers Surveyed', link: 'survey' },
    { icon: 'bi-box-seam-fill',           bg: 'bg-g',  col: 'col-g',  val: totalStock,     lbl: 'Total Available Stock', link: 'stock' },
    { icon: 'bi-dash-circle-fill',        bg: 'bg-y',  col: 'col-y',  val: underweight,    lbl: 'Underweight Children',  link: 'bmi' },
    { icon: 'bi-exclamation-triangle-fill', bg: 'bg-r', col: 'col-r', val: criticalNutr,   lbl: 'Critical Nutrition',    link: 'bmi' },
  ];

  document.getElementById('dash-stats').innerHTML = statsData.map(s => `
    <div class="scard" onclick="showPage('${s.link}')" style="cursor:pointer" title="Go to ${s.lbl}">
      <div class="scard-icon ${s.bg}">
        <i class="bi ${s.icon} ${s.col}" style="font-size:22px"></i>
      </div>
      <div>
        <div class="scard-val ${s.col}">${s.val}</div>
        <div class="scard-lbl">${s.lbl}</div>
      </div>
    </div>`).join('');

  // ── Alerts
  const lowItems = normalizedStocks.filter(s => s.qty <= s.minQty);
  const critBmi  = DB.bmiRecords.filter(r => r.category === 'Severe Underweight' || r.category === 'Underweight');
  let alertHtml = '';
  ...
  if (critBmi.length) {
    alertHtml += `
      <div class="bmi-alert-banner">
        <div class="bmi-alert-icon">⚠️</div>
        <div style="flex:1">
          <div class="bmi-alert-title">
            ${critBmi.length} Child${critBmi.length > 1 ? 'ren' : ''} Require Nutrition Attention
          </div>
          <div class="bmi-alert-kids">
            ${critBmi.slice(0,4).map(r => `<strong>${r.child_name}</strong> (${r.category})`).join(' &middot; ')}
            ${critBmi.length > 4 ? `<span style="color:var(--text-s)"> +${critBmi.length - 4} more</span>` : ''}
          </div>
        </div>
  ...
  if (!alertHtml) {
    alertHtml = `
      <div class="palert palert-ok">
        <i class="bi bi-check-circle-fill"></i>
        All systems normal. No urgent alerts today.
      </div>`;
  }
  document.getElementById('dash-notifs').innerHTML = alertHtml;

  // ── Stock overview
  document.getElementById('dash-stock').innerHTML = normalizedStocks.slice(0, 6).map(s => {
    const pct   = Math.min(100, Math.round((s.qty / (s.minQty * 5)) * 100));
    const color = s.qty <= s.minQty ? '#EF4444' : pct < 40 ? '#F59E0B' : '#10B981';
    const tag   = s.qty <= s.minQty
      ? '<span class="tag tag-r"><i class="bi bi-exclamation-circle"></i> Critical</span>'
      : pct < 40
        ? '<span class="tag tag-y">Low</span>'
        : '<span class="tag tag-g">Good</span>';
    return `
      <div class="srow">
        <div class="srow-name">${s.item_name}</div>
        <div class="srow-bar">
          <div class="pbar">
            <div class="pbar-fill" style="width:${pct}%;background:${color}"></div>
          </div>
        </div>
        <div class="srow-qty">${s.qty} ${s.unit}</div>
        <div>${tag}</div>
      </div>`;
  }).join('') || '<div class="palert palert-ok" style="margin:0"><i class="bi bi-info-circle-fill"></i> No stock items found.</div>';

  // ── Recent children
  const recent = DB.children.slice(0, 5);
  if (recent.length) {
    document.getElementById('dash-children').innerHTML = `
      <div class="table-responsive">
        <table class="ptable">
          <thead>
            <tr><th>#</th><th>Name</th><th>Age</th><th>Gender</th><th>Parent</th></tr>
          </thead>
          <tbody>
            ${recent.map((c, i) => `
              <tr>
    ...
    document.getElementById('dash-children').innerHTML =
      '<div class="palert palert-ok" style="margin:0"><i class="bi bi-info-circle-fill"></i> No children enrolled yet.</div>';
  }
}
...
function openVillagerModal(vid) {
  ...
  if (editVillagerId) {
    ...
    if (v) {
      ...
      document.getElementById('vm-age').value = v.age;
      document.getElementById('vm-gender').value = v.gender;
      document.getElementById('vm-category').value = v.category;
      document.getElementById('vm-contact').value = v.contact_number || '';
      document.getElementById('vm-addr').value = v.address || '';
    }
  } else {
    titleEl.innerHTML = `<i class="bi bi-person-plus-fill"></i> Add Villager`;
    document.getElementById('vm-name').value = '';
    document.getElementById('vm-age').value = '';
    document.getElementById('vm-gender').value = '';
    document.getElementById('vm-category').value = '';
    document.getElementById('vm-contact').value = '';
    document.getElementById('vm-addr').value = '';
  }
  
  const modal = new bootstrap.Modal(document.getElementById('villagerModal'));
  modal.show();
}

async function saveVillager() {
  const name = document.getElementById('vm-name').value.trim();
  const ageVal = document.getElementById('vm-age').value;
  const gender = document.getElementById('vm-gender').value;
  const category = document.getElementById('vm-category').value;
  const contact = document.getElementById('vm-contact').value.trim();
  const address = document.getElementById('vm-addr').value.trim();

  if (!name || ageVal === '' || !gender || !category) {
    toast('Name, Age, Gender and Category are required.', 'error');
    return;
  }

  const age = parseInt(ageVal, 10);
  if (isNaN(age) || age < 0) {
    toast('Age must be a positive integer.', 'error');
    return;
  }
  ...
  let res;
  if (editVillagerId) {
    res = await apiFetch(`/villagers/${editVillagerId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload)
    });
  } else {
    res = await apiFetch('/villagers', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload)
    });
  ...
  if (res.success) {
    toast(editVillagerId ? '✅ Villager updated successfully' : '✅ Villager registered successfully', 'success');
    
    // Close modal
    const modalEl = document.getElementById('villagerModal');
    const modal = bootstrap.Modal.getInstance(modalEl);
    if (modal) modal.hide();

    // Reload villagers list
    const fetchRes = await apiFetch('/villagers');
    if (fetchRes.success) {
      DB.villagers = fetchRes.data || [];
      
      // Reset filters and search
      document.getElementById('villager-search').value = '';
      document.getElementById('villager-filter-category').value = '';
      renderVillagersTable();
    ...
    toast(res.message || 'Failed to save villager record', 'error');
  }
}

async function deleteVillager(vid) {
  const v = DB.villagers.find(item => String(item.id) === String(vid));
  if (!v) return;

  if (!confirm(`Are you sure you want to delete villager "${v.name}" from the registry?`)) return;

  const res = await apiFetch(`/villagers/${vid}`, {
    method: 'DELETE'
  });

  if (res.success) {
    toast('✅ Villager record deleted successfully', 'success');
    ...
    toast(res.message || 'Failed to delete villager record', 'error');
  }
}

function downloadVillagersPdf() {
  downloadBackendReport('villagers');
}

function autoFillSurveyFromRegistry() {
  if (!DB.villagers || DB.villagers.length === 0) {
    toast('No villagers registered in the registry to auto-fill metrics from.', 'warning');
    return;
  }
  
  // Calculate counts
  const totalPopulation = DB.villagers.length;
  
  // Families: count unique addresses or default to population / 4
  const uniqueAddresses = new Set();
  DB.villagers.forEach(v => {
    if (v.address && v.address.trim()) {
      uniqueAddresses.add(v.address.trim().toLowerCase());
    }
  });
  const totalFamilies = uniqueAddresses.size > 0 ? uniqueAddresses.size : Math.max(1, Math.ceil(totalPopulation / 4));
  
  const totalChildren = DB.villagers.filter(v => v.category === 'Child').length;
  const pregnantWomen = DB.villagers.filter(v => v.category === 'Pregnant Woman').length;
  const lactatingMothers = DB.villagers.filter(v => v.category === 'Lactating Mother').length;
  
  // Auto-fill values
  document.getElementById('vs-population').value = totalPopulation;
  document.getElementById('vs-families').value = totalFamilies;
  document.getElementById('vs-children').value = totalChildren;
  document.getElementById('vs-pregnant').value = pregnantWomen;
  document.getElementById('vs-lactating').value = lactatingMothers;
  
  // Auto fill village name from first villager address or profile center
  const firstAddress = DB.villagers.find(v => v.address && v.address.trim())?.address;
  if (firstAddress && !document.getElementById('vs-village-name').value.trim()) {
    // try to extract village name or use full address
    const parts = firstAddress.split(',');
    document.getElementById('vs-village-name').value = parts[parts.length - 1].trim();
  } else if (DB.user && DB.user.village && !document.getElementById('vs-village-name').value.trim()) {
    document.getElementById('vs-village-name').value = DB.user.village;
  }
  
  toast('🎯 Demographic counts auto-filled from Villagers Registry!', 'success');
}
